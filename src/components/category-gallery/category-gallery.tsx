        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {galleryImages.map((src, idx) => (
              <div
                key={idx}
                className="group relative h-64 w-full overflow-hidden rounded-xl shadow-lg hover:shadow-xl"
              >
                {/* 4:3 aspect ratio box (height = 75% of width) */}
                {/* <div className="pt-[75%]" /> */}

                {/* absolutely fill the box */}
                <div className="group relative w-full h-64 overflow-hidden rounded-xl shadow-lg hover:shadow-xl">
  <img
    src={src}
    alt={`${category}-${idx + 1}`}
